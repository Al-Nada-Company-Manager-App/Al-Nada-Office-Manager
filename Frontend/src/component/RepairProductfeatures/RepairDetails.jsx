...
const RepairDetails = ({selectedRepair, setSelectedRepair,detailRepairModalVisible, handleModalClose, fetchAllDUM, onDelete,fetchSpareParts}) => {
  ...
  const fetchUpdatedStatus = async (p_id) => {
    try {
      ...
      const response = await axios.get(`http://localhost:4000/api/getProductStatus/${p_id}`);
  ...
  useEffect(() => {
    ...
    return (
        <> {selectedRepair && (
            ...
            footer={[
              <>
              <Button key="Delete" onClick={() => {handleDeleteRepairProcess(selectedRepair.rep_id)}} danger>
                Delete
              </Button>
              ...
              <div>
                <Row gutter={16}>
                  ...
                  <div className="product-details">
                    <p><strong>Product Name:</strong> {selectedRepair.p_name}</p>
                    <p><strong>Product Category:</strong> {selectedRepair.p_category}</p>
                    <p><strong>Serial Number:</strong> {selectedRepair.serial_number}</p>
                    <p><strong>Status:</strong> {selectedRepair.p_status}</p>
                  </div>
                        </Col>
                </Row>
