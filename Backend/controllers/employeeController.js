...
const signedUser = async (req, res) => {
  try {
    ...
    console.error("Error in signedUser controller:", error); // Log the error
...
const getAllEmployees = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Internal server error1321" });
  }
};

const getEmployeeAccess = async (req, res) => {
  try {
    const { id } = req.params;
    if (!id) {
